...
def wompi_webhook():
    ...
    logger.info("Transacción: id=%s | ref=%s | estado=%s | monto=%s centavos",
                transaction_id, reference, status, amount_in_cents)

    if not ODOO_REF_PATTERN.match(reference):
        logger.info("Referencia '%s' no es formato Odoo. Ignorado.", reference)
        return Response(status=200)
